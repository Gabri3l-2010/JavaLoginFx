...
import org.gabrielchiu.dao.UsuarioDAO;
import org.gabrielchiu.model.Usuario;
import org.gabrielchiu.util.SecurityUtil;
...
public class InicioSesionController implements Initializable {
    ...
    private void abrirDashboard(Usuario usuario) {
        ...
        switch (usuario.getRol().toLowerCase()) {
            case "admin":
                rutaFXML = "/org/gabrielchiu/view/style/AdminDashboradView.fxml";
                tituloDashboard = "Panel de Administrador";
                break;
            case "empleado":
                rutaFXML = "/org/gabrielchiu/view/style/EmpleadoDashboardView.fxml";
                tituloDashboard = "Panel de Empleado";
                break;

            case "cajero":
                rutaFXML = "/org/gabrielchiu/view/style/CajeroDashboardView.fxml";
        ...
        try {
            ...
            if (controlador instanceof AdminDashboardController) {
                ((AdminDashboardController) controlador).iniciarUsuario(usuario);
            } else if (controlador instanceof EmpleadoDashboardController) {
                ((EmpleadoDashboardController) controlador).iniciarUsuario(usuario);
            } else if (controlador instanceof CajeroDashboardController) {
                ((CajeroDashboardController) controlador).iniciarUsuario(usuario);
            }
            ...
            e.printStackTrace();
            if (lblMensaje != null) {
                lblMensaje.setText("Error al abrir la vista.");
            }
        }
    }
}
